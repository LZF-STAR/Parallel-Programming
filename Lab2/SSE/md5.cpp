...
void MD5Hash(string input[4], __m128i *state)
{
	...
	for (int i = 0; i < 4; ++i)
	{
		blocks[i] = messageLength[i] / 64;
	}
	int max_blocks = *max_element(blocks, blocks + 4);
	...
	for (int i = 0; i < max_blocks; i += 1)
	{
		...
		for (int j = 0; j < 16; ++j)
		{
			...
			for (int m = 0; m < 4; ++m)
			{
				if (i < blocks[m])
				{
					Byte *msg = paddedMessage[m];
					int offset = i * 64 + j * 4;
					lanes[m] = (uint32_t)msg[offset] | ((uint32_t)msg[offset + 1] << 8) | ((uint32_t)msg[offset + 2] << 16) | ((uint32_t)msg[offset + 3] << 24);
				}
				else
				{
					lanes[m] = 0;
			...
			x[j] = _mm_set_epi32(lanes[3], lanes[2], lanes[1], lanes[0]);
		}
	...
	// 下面的处理，在理解上较为复杂
	// 字节顺序反转
	static const __m128i byte_swap_mask = _mm_setr_epi8(
		3, 2, 1, 0,	   // lane0: b3,b2,b1,b0
		7, 6, 5, 4,	   // lane1: b7,b6,b5,b4
		11, 10, 9, 8,  // lane2: b11,b10,b9,b8
		15, 14, 13, 12 // lane3: b15,b14,b13,b12
	);
	for (int i = 0; i < 4; ++i)
	{
		state[i] = _mm_shuffle_epi8(state[i], byte_swap_mask);
	}
