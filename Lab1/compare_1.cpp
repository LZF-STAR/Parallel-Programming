...
//平凡算法,逐列访问
double use_cache_me(const double* a, const double* b, double* ans,int n,int r) {
	double total_time;
...
//类比ppt中的代码
double use_cache_AI(const double* a, const double* b, double* ans,int n,int r) {
	double total_time;
	long long head, tail, freq;
	QueryPerformanceFrequency((LARGE_INTEGER*)&freq);
	QueryPerformanceCounter((LARGE_INTEGER*)&head);
...
int main() {
	...
	for (int i = 1000; i <= 15000; i += 1000) {
		ress1.push_back(use_cache_me(a,b,ans1,i,5));//5次实验
		ress2.push_back(use_cache_AI(a,b,ans2,i,5));
	}
	cout << "接下来依次输出的是：我的优化算法所用时间（ms），大模型提示我的优化算法所用时间（ms），运行速度提升倍数";
	cout << endl;
