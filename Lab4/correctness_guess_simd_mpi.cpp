...
// 编译指令
// mpic++ -O2 correctness_guess_simd_mpi.cpp train.cpp guessing_mpi.cpp md5_neon.cpp -o main -O2
    ...
    while (continue_running)
    {
        ...
        if (rank == 0)
        {
            ...
            if (curr_num > 1000000)
            {
                ...
                for (auto &pw : all_pw)
                {
                    if (!pw.empty() && test_set.find(pw) != test_set.end())
                    {
                        cracked += 1;
                    }
                }
